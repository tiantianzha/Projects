...
def read_callspan():
    ...
    global log_data
    log_data = [dict() for i in range(person_count + 1)]
    global exists1
    exists1 = [0] * (person_count + 1)
    global sum_msg1
    sum_msg1 = [0] * (person_count + 1)
    global sum_data1
    sum_data1 = [0] * (person_count + 1)
    ...
    for i in range(1, person_count + 1):
        ...
        while(1):
            ...
            elif line['description'] == "Packet Data":
                log_data[i][line['phonenumber_oid']] = log_data[i].get(line['phonenumber_oid'], 0) + 1
        # display times as a percentage of total time
        sum_msg1[i] = sum(log_msg[i].itervalues())
        sum_data1[i] = sum(log_data[i].itervalues())
        for k, v in log_msg[i].iteritems():
            log_msg[i][k] = float(log_msg[i][k]) / sum_msg1[i]
        for k, v in log_data[i].iteritems():
            log_data[i][k] = float(log_data[i][k]) / sum_data1[i]
    ...
    for i in range(1, person_count + 1):
        ...
        while(1):
            ...
            if line['description'] == "Short Message":
                ...
                anon_data[i][line['phonenumber_oid']] = anon_data[i].get(line['phonenumber_oid'], 0) + 1
        # sum over all values->get percentages
        sum_msg2[i] = sum(anon_msg[i].itervalues())
        sum_data2[i] = sum(anon_data[i].itervalues())
            #if sum_msg2[i] == 0 or sum_data2[i] == 0:
    #continue
        for k, v in anon_msg[i].iteritems():
            anon_msg[i][k] = float(anon_msg[i][k]) / sum_msg2[i]
        for k, v in anon_data[i].iteritems():
            anon_data[i][k] = float(anon_data[i][k]) / sum_data2[i]
        ...
        for j in range(1, person_count + 1):
            ...
            if exists1[j]:
                ...
                hamming_data[i][j] = compute_hamming(anon_data[i], log_data[j])
            else:
                hamming_msg[i][j] = float("inf")
                hamming_data[i][j] = float("inf")
            # update min if necessary
            if hamming_data[i][j] == 0:
                hamming[i][j] = hamming_msg[i][j] *2
            else:
                hamming[i][j] = hamming_msg[i][j] + hamming_data[i][j]
...
def output_results():
    ...
    for i in range(1, person_count + 1):
        if exists2[i] == 1 and exists1[i] == 1:
            ...
            avg_msg1 += sum_msg1[i]
            avg_msg2 += sum_msg2[i]
            avg_data1 += sum_data1[i]
            avg_data2 += sum_data2[i]
            valid.append(i)
    ...
    avg_data1 = float(avg_data1) / count
    avg_data2 = float(avg_data2) / count
    ...
    for i in valid:
        if i == min_index[i]:
            ...
            avg_data_correct1 += sum_data1[i]
            avg_data_correct2 += sum_data2[i]
        else:
            avg_msg_wrong1 += sum_msg1[i]
            avg_msg_wrong2 += sum_msg2[i]
            avg_data_wrong1 += sum_data1[i]
            avg_data_wrong2 += sum_data2[i]
    ...
    avg_data_correct1 = float(avg_data_correct1) / correct_count
    avg_data_correct2 = float(avg_data_correct2) / correct_count
    avg_data_wrong1 = float(avg_data_wrong1) / (count - correct_count)
    avg_data_wrong2 = float(avg_data_wrong2) / (count - correct_count)
    ...
    print ""
    print "All items: average data 2nd sem {0}\n           average data 1st sem {1}".format(avg_data2, avg_data1)
    print "Correct items: average data 2nd sem {0}\n               average data 1st sem {1}".format(avg_data_correct2, avg_data_correct1)
    print "Wrong items: average data 2nd sem {0}\n             average data 1st sem {1}".format(avg_data_wrong2, avg_data_wrong1)
    print ""
    ...
    for i in valid:
        ...
        if i != min_index[i]:
            ...
            print "2nd sem data: {0}".format(sum_data2[i])
            print "1st sem returned value data: {0}".format(sum_data1[min_index[i]])
            print "1st sem actual value data: {0}".format(sum_data1[i])
        else:
            print "CORRECT"
            ...
            print "2nd sem data: {0}".format(sum_data2[i])
            print "1st sem data: {0}".format(sum_data1[i])

        # print sorted list of hamming values
        print " total, msg, data"
        for tup in sorted_hamming:
            print "{0}: {1}      {2} + {3}".format(tup[0], tup[1], hamming_msg[i][tup[0]], hamming_data[i][tup[0]])
